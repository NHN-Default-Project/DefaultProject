...
public class BlackjackGame {
    ...
    private final BlackjackHand userHand;
    private boolean isUserWin;
    private boolean isGameEnd;
    private boolean isUserFirstPick;
    ...
    public void viewOnesHand(String playerType, BlackjackHand playerHand) {
        // 플레이어의 모든 카드를 본다
        System.out.println("--------------------------------------");
        for (int i = 0; i < playerHand.getCardCount(); i++) {
            System.out.println(playerType + " 의 " + (i + 1) + " 번째 카드 : " + playerHand.getCard(i));
        }
        System.out.println("--------------------------------------");
    }
    ...
    public void viewOneCardOfDealer() {
        ...
        System.out.println("Dealer 의 " + 1 + " 번째 카드 : " + this.dealerHand.getCard(0));
    ...
    public void hit() {
        if (this.isUserFirstPick) {
            this.userHand.addCard(deck.dealCard());

            if (this.userHand.getBlackjackValue() > 21) {
                this.isUserWin = false;
                this.isGameEnd = true;
            }
        } else {
            ...
            viewOnesHand("Dealer", dealerHand);

            if (this.dealerHand.getBlackjackValue() > 21) {
                this.isUserWin = true;
            } else if (this.dealerHand.getBlackjackValue() >= this.userHand.getBlackjackValue()) {
                this.isUserWin = false;
            } else {
                this.isUserWin = true;
            }
    ...
    public void stand() {
        ...
        if (this.dealerHand.getBlackjackValue() <= 16) {
    ...
    public void chooseHitOrStand(Scanner sc) {
        ...
        do {
            this.viewOnesHand("User", userHand);
