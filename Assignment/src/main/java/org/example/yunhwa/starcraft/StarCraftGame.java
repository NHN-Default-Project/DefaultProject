...
public class StarCraftGame {
    ...
    public void chooseOnesArmyAndEnemyUntilEnd(Scanner sc) {
        ...
        while (true) {
            ...
            int currentDefensePower;
            String attackedUnitName;
            ...
            // 공격 가능한 것이 있는지 체크
            for (Unit thisUserUnit : this.userUnitList) {
                // 나는 유닛 공격 불가능한 유닛이라면
                if (thisUserUnit instanceof NotFlyable && (!((NotFlyable) thisUserUnit).canAttackFlyable())) {
                    // 모든 적군 유닛들 확인하면서 하나라도 안나는 유닛 있다면 공격 가능
                    for (Unit thisComputerUnit : this.computerUnitList) {
                        if (thisComputerUnit instanceof NotFlyable) {
                            canAttack = true;
                            break; // 2차 for 문 break
                        }
                    }

                } else { // 애초에 공격 가능한 거라면 그냥 빠져나오기
                    canAttack = true;
                    break; // 1차 for 문 break
                }
            }


            if (!canAttack) {
                // 모든 아군 유닛들이 적군 유닛들을 다 공격하지 못하는 경우
                System.out.println("모든 아군 유닛들이 적군 유닛들을 전부 공격하지 못하는 상황입니다. 컴퓨터 턴으로 변경하겠습니다.");

            } else { // 공격할 유닛들 선택 후 공격 및 게임 종료 체크

                try {
                    while (true) { // 유저가 공격할 유닛들 선택 가능할 때까지 선택
                    ...
                    // 선택 완료! 공격 하기
                    currentDefensePower = this.computerUnitList.get(attackedUnitIndex).getDefensePower();
                    ...
                    attackedUnitName = this.computerUnitList.get(attackedUnitIndex).getClass().getSimpleName();
                    ...
                    // user 입장에서의 적군(컴퓨터) 의 모든 유닛이 파괴되었다면 유저 승리
            ...
            for (Unit thisComputerUnit : this.computerUnitList) {
                // 나는 유닛 공격 불가능한 유닛이라면
                if (thisComputerUnit instanceof NotFlyable && (!((NotFlyable) thisComputerUnit).canAttackFlyable())) {
                    // 모든 적군 유닛들 확인하면서 하나라도 안나는 유닛 있다면 공격 가능
                    for (Unit thisUserUnit : this.userUnitList) {
                        if (thisUserUnit instanceof NotFlyable) {
                            canAttack = true;
                            break; // 2차 for 문 break
                        }
                    }
            ...
            if (!canAttack) {
                // 모든 아군 유닛들이 적군 유닛들을 다 공격하지 못하는 경우
                System.out.println("모든 아군 유닛들이 적군 유닛들을 전부 공격하지 못하는 상황입니다. 유저 턴으로 변경하겠습니다.");
            } else {


                while (true) {
                    // 공격할 유닛 선택
                    System.out.println("컴퓨터 차례! 공격을 수행할 아군 유닛과 공격할 적군 유닛을 선택해보겠습니다 :");
                    ...
                    System.out.println(attackUnitIndex + " " + attackedUnitIndex);
                ...
                // 이제 공격 시도 시작!
                System.out.printf("컴퓨터가 공격을 수행할 아군 유닛과 공격할 적군 유닛을 선택한 결과: %d %d %n", attackUnitIndex, attackedUnitIndex); // 0 3
                ...
                // user 입장의 아군(유저)의 모든 유닛이 파괴되었다면 컴퓨터 승리
