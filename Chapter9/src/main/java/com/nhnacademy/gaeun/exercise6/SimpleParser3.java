...
public class SimpleParser3 {
    ...
    private static class ConstNode extends ExpNode {
        ...
        void printInfix() {
            System.out.printf("(" + derivative.value(0) + ")");
        }
     ...
     */
    private static class BinOpNode extends ExpNode { //이진연산
        ...
        double value(double value) {
            this.value = value;
            System.out.println("x: " + left.value(value) + "y: " + right.value(value));
        ...
        ExpNode derivative() {
            switch (op) {
                ...
                default:
                    return new ConstNode(Double.NaN);  // Bad operator!
            }
        }
    ...
    private static ExpNode factorTree() throws ParseError {
        ...
        if (Character.isDigit(ch)) {
            ...
            return new ConstNode(TextIO.getDouble());
        } else if (ch == 'x' || ch == 'X') {
            TextIO.getAnyChar();
